...
public class EasySwipeMenuLayout extends ViewGroup {
    ...
    private static final String TAG = EasySwipeMenuLayout.class.getSimpleName();
    ...
    public EasySwipeMenuLayout setOnSwipeListener(OnSwipeListener onSwipeListener) {
        this.onSwipeListener = onSwipeListener;
        return this;
    }

    public interface OnSwipeListener {
        void onSwipe(@SwipeState int state);
    }
    ...
    public void handlerSwipeMenu(@SwipeState int result) {
        if (result == SwipeState.LEFTOPEN) {
            mScroller.startScroll(getScrollX(), 0, mLeftView.getLeft() - getScrollX(), 0);
            mViewCache = this;
            mStateCache = result;
            if (onSwipeListener != null) {
                onSwipeListener.onSwipe(SwipeState.LEFTOPEN);
            }
        } else if (result == SwipeState.RIGHTOPEN) {
            mViewCache = this;
            mScroller.startScroll(getScrollX(), 0, mRightView.getRight() - mContentView.getRight() - mContentViewLp.rightMargin - getScrollX(), 0);
            mStateCache = result;
            if (onSwipeListener != null) {
                onSwipeListener.onSwipe(SwipeState.RIGHTOPEN);
            }
        } else {
            mScroller.startScroll(getScrollX(), 0, -getScrollX(), 0);
            mViewCache = null;
            mStateCache = SwipeState.NULL;
    ...
    public void resetStatus() {
        if (mViewCache != null) {
            if (mStateCache != SwipeState.NULL && mStateCache != SwipeState.CLOSE && mScroller != null) {
