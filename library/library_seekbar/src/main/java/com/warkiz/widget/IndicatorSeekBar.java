...
public class IndicatorSeekBar extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mAdjustAuto = ta.getBoolean(R.styleable.IndicatorSeekBar_isb_thumb_adjust_auto, false);
    ...
    private boolean autoAdjustThumb() {
        ...
        if (mTicksCount < 3 || !mAdjustAuto) {//it is not necessary to adjust while count less than 2.
            return false;
        }
    ...
    public synchronized void setProgress(float progress) {
        ...
        if ((!mSeekSmoothly) && mTicksCount > 2 && mAdjustAuto) {
            mProgress = mProgressArr[getClosestIndex()];
        }
        setSeekListener(false);
        refreshThumbCenterXByProgress(mProgress);
    ...
    /**
     * Set  track progress color
     * R.styleable.IndicatorSeekBar_isb_track_progress_color
     *
     * @param progressTrackColor the color for track progress
     */
    public void setTrackProgressColor(@ColorInt int progressTrackColor) {
        mProgressTrackColor = progressTrackColor;
        invalidate();
    }

    /**
     * Set  track indicator color
     * R.styleable.IndicatorSeekBar_isb_indicator_color
     *
     * @param indicatorColor the color for indicator color
     */
    ...
    public void setThumbSize(int thumbSize) {
        mThumbSize = SizeUtils.dp2px(getContext(), thumbSize);
        invalidate();
    }
