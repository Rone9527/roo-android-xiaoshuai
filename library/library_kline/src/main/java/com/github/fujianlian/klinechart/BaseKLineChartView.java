...
import com.github.fujianlian.klinechart.formatter.DateTimeFormatter;
...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //右边移动最大设置一个列宽度
        setOverScrollRange(mWidth / mGridColumns);
        //初始化默认滚动设置一个列宽度
    ...
    private void initRect() {
        if (isShowChild) {
            int mMainHeight = (int) (displayHeight * (mVolDraw == null ? 0.75f : 0.6f));
            int mVolHeight = (int) (displayHeight * (mVolDraw == null ? 0 : 0.2f));
            int mChildHeight = (int) (displayHeight * (mVolDraw == null ? 0.25f : 0.2f));
            ...
            int mMainHeight = (int) (displayHeight * (mVolDraw == null ? 1 : 0.75f));
            int mVolHeight = (int) (displayHeight * (mVolDraw == null ? 0 : 0.25f));
            mMainRect = new Rect(0, mTopPadding, mWidth, mTopPadding + mMainHeight);
            mVolRect = new Rect(0, mMainRect.bottom + mChildPadding, mWidth, mMainRect.bottom + mVolHeight);
        }
    }
    ...
    private void drawGird(Canvas canvas) {
        ...
        if (mChildDraw != null) {
            bgPaint = new Paint();
            bgPaint.setColor(Color.parseColor("#5ca0fd"));
            bgPaint.setAlpha(10);
            //画volume背景
            canvas.drawRect(0, mMainRect.bottom, mWidth, mChildRect.bottom, bgPaint);

            //画时间的背景
            bgPaint.setColor(Color.parseColor("#FF0A0F1E"));
            bgPaint.setAlpha(255);
            canvas.drawRect(0, mChildRect.bottom, mWidth, mChildRect.bottom + mBottomPadding, bgPaint);
            ...
            bgPaint.setAlpha(10);
            //画volume背景
            canvas.drawRect(0, mMainRect.bottom, mWidth, mVolRect.bottom, bgPaint);

            //画时间的背景
            bgPaint.setColor(Color.parseColor("#FF0A0F1E"));
            bgPaint.setAlpha(255);
            canvas.drawRect(0, mVolRect.bottom, mWidth, mVolRect.bottom + mBottomPadding, bgPaint);
    ...
    private void drawK(Canvas canvas) {
        ...
        if (isLongPress) {
            ...
            // k线图竖线
            canvas.drawLine(x, mMainRect.top, x, mMainRect.bottom, mSelectedXLinePaint);// 换成xline paint
            // k线图横线
            canvas.drawLine(-mTranslateX, y, -mTranslateX + mWidth / mScaleX, y, mSelectedXLinePaint);
            // 柱状图竖线
            canvas.drawLine(x, mMainRect.bottom, x, mVolRect.bottom, mSelectedXLinePaint);
            if (mChildDraw != null) {
                // 子线图竖线
                canvas.drawLine(x, mVolRect.bottom, x, mChildRect.bottom, mSelectedXLinePaint);
            }
        }
        if (mMainDraw != null && ((MainDraw) mMainDraw).isLine() && mStopIndex == mItemCount - 1) {
            drawMainMinuteDot(canvas, ((MainDraw) mMainDraw).getLinePaint(), mStopIndex, ((ICandle) getItem(mStopIndex)));
        }
     ...
     * 在主区域画分时线最后的点
     */
    public void drawMainMinuteDot(Canvas canvas, Paint paint, int position, ICandle finalPoint) {
        canvas.scale(1, 1);
        Path path5 = new Path();
        path5.addCircle(getX(position), getMainY(finalPoint.getClosePrice()), 6, Path.Direction.CCW);
    ...
    private void drawText(Canvas canvas) {
        ...
        //--------------画中间子图的值-------------
        if (mVolDraw != null) {
            String volume = mVolDraw.getValueFormatter().format(mVolMaxValue);
            canvas.drawText(volume,
                    mWidth - calculateWidth(volume), mMainRect.bottom + baseLine, mTextPaint);
...
//			canvas.drawText(date, x - textWidth / 2, y + baseLine + 5, mTextPaint);
    ...
    private void drawValue(Canvas canvas, int position) {
        ...
        if (position >= 0 && position < mItemCount) {
            if (mMainDraw != null) {
                ...
                //更改MA文字的偏移值
                mMainDraw.drawText(canvas, this, position, dp2px(5), y);
            }
            if (mVolDraw != null) {
                float y = mMainRect.bottom + baseLine;
                mVolDraw.drawText(canvas, this, position, dp2px(5), y);
            }
            if (mChildDraw != null) {
     ...
     */
    public void changeMainDrawType(@Status int status) {
    ...
    public String formatDateTime(Date date) {
        if (getDateTimeFormatter() == null) {
            setDateTimeFormatter(new DateTimeFormatter());
        }
