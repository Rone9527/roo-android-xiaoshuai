...
public class ProtocalDialog extends FullScreenDialogFragment {
    private OnEnsureClickedListener onEnsureClickedListener;

    public static ProtocalDialog newInstance() {
        ProtocalDialog fragment = new ProtocalDialog();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View inflate = inflater.inflate(R.layout.dialog_protocal, container, false);

        RadiusTextView ensure = inflate.findViewById(R.id.btn_ensure);
        RadiusCheckBox checkbox = inflate.findViewById(R.id.checkbox);
        checkbox.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ensure.setEnabled(isChecked);
        });
        RxView.clicks(ensure).throttleFirst(1, TimeUnit.SECONDS)
                .subscribe(o -> {
                    if (ensure.isEnabled()) {
        ...
        tvContent.setText(PROTACAL_SERVICE + "\n\n" + PROTACAL_PRAVICY);
        return inflate;
    }
    ...
    public ProtocalDialog setOnEnsureClickedListener(OnEnsureClickedListener onEnsureClickedListener) {
    ...
    public static final String PROTACAL_PRAVICY = "《Roo Wallet隐私政策》\n" +
            "\n" +
            "尊敬的用户：\n" +
    ...
    public static final String PROTACAL_SERVICE = "《Roo Wallet服务协议》\n" +
            "最近更新于：2021年08月4日\n" +
            "尊敬的用户：\n" +
            "感谢您选择Roo Wallet服务。Roo Wallet 是Water Source Foundation Limited 旗下的一款加密货币钱包产品。\n" +
            "《Roo Wallet服务协议》（以下简称“本协议”）由 Water Source Foundation Limited （以下简称“本公司”，和您（以下简称“用户”）签订，本协议在您与本公司之间具有合同上的法律效力。在本协议中：（1）“我们”指代本公司，“我们的”应据此解释；及（2）“您”指代用户，“您的”应据此解释。您和本公司单独称为“一方”，合称为“双方”。\n" +
            "本公司在此特别提醒您在使用我们的Roo Wallet移动应用（以下简称“Roo Wallet” 或“本软件”，Roo Wallet可在各移动应用平台上下载，包括但不限于Google Play和Apple App Store）之前，请认真阅读本协议及后文提及的相关协议，尤其是本协议中“免责及责任限制”等以加粗形式体现的条款，确保您充分理解本协议中各条款，并自主考虑风险。\n" +
            "一、 关于本协议的确认与接纳\n" +
            "1.您理解本协议及有关协议适用于Roo Wallet及Roo Wallet上本公司所自主开发和拥有的去中心化应用（简称“DApp”）（排除第三方开发的DApp）。\n" +
            "2.您下载Roo Wallet软件并创建身份、恢复身份（定义见下文）或导入钱包，即视为您已经充分阅读并接受本协议全部条款，本协议立即生效，对双方具有约束力。 如果您不同意本协议条款，您应立即停止使用Roo Wallet。如果您已经下载了Roo Wallet，请立即删除。\n" +
            "3.在访问或使用Roo Wallet时, 您同意：\n" +
            "（1）接受本协议最新版本的约束（不变更亦不修改）；\n" +
            "（2）在您所适用的司法管辖区域内，您已达到使用Roo Wallet的法定年龄，并可承担因使用Roo Wallet而产生的有约束力法律或金钱义务；且\n" +
            "（3）您不属于被排除人士（如本协议所定义）。\n" +
            "4.本协议可由本公司随时更新，经修改的协议一经在Roo Wallet上公布，立即自动生效，不再另行通知。在本公司公布修改协议条款后，如果您不接受修改后的条款，请立即停止使用Roo Wallet，您继续使用Roo Wallet将被视为接受修改后的协议。\n" +
            "二、 定义\n" +
            "1. Roo Wallet： 指由本公司基于区块链开发的数字钱包，包括其他为方便用户使用区块链系统而开发的辅助工具。\n" +
            "2.被排除人士：\n" +
            ...
            "4.新手指南： 指在您使用Roo Wallet之前（及在将来的使用过程中），本公司为您提供的Roo Wallet操作指南，以帮助您了解区块链的基础知识。\n" +
            "5.创建或导入钱包： 指在您接受本协议后，使用Roo Wallet创建或导入钱包的过程。\n" +
            "6.钱包密码： 指您在创建Roo Wallet钱包过程中，由您决定的密码，该密码将被用于加密和保护您的私钥。Roo Wallet作为去中心化的应用，钱包密码不存储在本公司的服务器，一旦您丢失或忘记钱包密码，您需要借助私钥或助记词重置钱包密码。\n" +
            "7.信息提示： Roo Wallet软件操作界面涉及的信息提示内容，建议用户按照相关步骤进行操作。\n" +
            "8.第三方服务： 指第三方DApp、第三方智能合约、第三方开源协议、第三方硬件钱包、第三方网页、第三方交易所等第三方提供的产品和服务。\n" +
            "9.特定用户： 指按照各个国家法律法规及政策规定必须要配合本公司履行个人信息披露义务的用户。\n" +
            "10.开发者用户： 指按照本公司通知及相关开源协议规定使用Roo Wallet开源代码和开发者模式等针对开发者提供的服务的用户。\n" +
            "11.私钥： 由256位随机字符构成，是用户拥有并使用数字代币的核心，本公司不储存任何用户私钥或助记词信息。\n" +
            "12.公钥： 由私钥借助密码学原理单向推导生成，并用以生成区块链数字钱包地址，数字钱包地址即为公开收款地址。\n" +
            "13.助记词： 符合区块链BIP39 行业标准，由随机算法生成的12（或15/18/21/24）个有序单词组成。是私钥的易记录表现形式，方便用户备份保管，本公司不储存任何用户私钥或助记词信息。\n" +
            "14.Keystore: 是私钥或助记词经过用户设置的钱包密码加密保存的文件形式，它只存储在您的这台移动设备中，不会同步至本公司服务器。\n" +
            "15.数字代币： 指Roo Wallet目前支持的数字代币种类，包括但不限于ETH、BTC等。\n" +
            "16.资料： 指Roo Wallet上的文章等资料，这些资料均为Roo Wallet所有，未经授权不得转载。这些资料仅供您参考，并不构成本公司对您交易的指导或建议。\n" +
            "17.消息中心： 指用户可以接收本公司发出的信息或通知的收件箱。\n" +
            "18.个人信息： 指以电子或者其他方式记录的能够单独或者与其他信息结合识别用户个人身份的各种信息，钱包地址、移动设备信息等，但不包括用户的钱包密码、私钥、助记词。\n" +
            "19.智能合约： 指一种旨在以信息化方式传播、验证或执行合约的、基于区块链的智能化合约。\n" +
            "20.Roo Wallet开源代码： 指本公司公开宣布开源的部分Roo Wallet软件代码，允许开发者用户在遵循相关开源协议及本公司通知的规定下使用（包括二次开发）该部分开源代码。\n" +
            "21.开源协议： 指开发者用户使用Roo Wallet开源代码所应遵循的第三方开源协议。\n" +
            "\n" +
            "三、Roo Wallet服务（统称“服务”）\n" +
            "1.创建钱包。您可以使用Roo Wallet提供的“创建钱包”并能同时管理多链钱包。\n" +
            "2.导入钱包。对Roo Wallet支持的数字代币，您可以使用Roo Wallet生成新钱包或导入相关区块链系统的其它钱包工具生成的兼容钱包。\n" +
            "3.转账、收款。您可以使用Roo Wallet的转账、收款功能进行数字代币的管理，对相关区块链的账本进行修改。转账是指付款方利用收款方的区块链地址进行转账操作，该“转账”行为涉及在相关区块链系统的分布式账本中对该交易的有效记录，而非在Roo Wallet上实际交付或转让数字代币。\n" +
            "4.管理数字代币。您可以从Roo Wallet操作界面添加、保管或移除Roo Wallet所支持的数字代币\n" +
            "5.闪兑。用户之间可以在智能合约上发生并完成的数字代币兑换，由Roo Wallet 作为界面工具帮助用户和智能合约交互并显示该用户之间兑换的相应结果。\n" +
            "6.币币兑换。用户可以与使用智能合约的第三方进行数字代币兑换，由Roo Wallet 作为界面工具帮助用户和智能合约交互并显示该数字代币兑换的相应结果。\n" +
            "7.浏览DApp。用户通过Roo Wallet访问并使用该DApp（包括本公司自己的DApp和第三方DApp）提供的服务。\n" +
            "8.搜索DApp。用户通过Roo Wallet上的搜索框，可以对已在Roo Wallet 罗列的DApp进行搜索。\n" +
            "9.交易记录。我们将通过区块链系统显示的交易记录。\n" +
            "10.其他本公司认为有必要提供的服务。\n" +
            "\n" +
            ...
            "（4）挂失钱包；\n" +
            "（5）恢复钱包；\n" +
            "2.您应当自行承担保管含有Roo Wallet的移动设备、备份Roo Wallet、备份钱包密码、助记词、私钥的责任。如您遗失移动设备、删除且未备份Roo Wallet、删除且未备份钱包、钱包被盗或遗忘钱包密码、私钥、助记词或Keystore，本公司均无法还原钱包或找回钱包密码、私钥、助记词或；如用户进行交易时误操作（例如输错转账地址、输错兑换数额），本公司无法取消交易，且本公司亦不应对此承担任何责任。\n" +
            "3.Roo Wallet上集成的DApp包括本公司自主拥有的DApp和第三方平台提供的DApp。对于第三方平台提供的DApp，Roo Wallet仅为用户进入DApp提供搜索和区块链浏览器功能，不对第三方DApp的功能或服务质量提供任何担保。用户在第三方DApp上接受服务或进行交易前应自行判断和评估该第三方DApp提供的服务或交易是否存在风险。\n" +
            "4.Roo Wallet上提供的兑换功能的实质是用户使用第三方智能合约在区块链系统上进行的数字代币兑换。对于第三方智能合约提供的服务，Roo Wallet仅为用户提供第三方智能合约的使用工具，不对第三方智能合约的功能或服务质量提供任何担保。用户在该第三方智能合约上接受服务或进行交易前应自行判断和评估该第三方智能合约提供的服务或交易是否存在风险。用户应仔细阅读并同意相关智能合约的不断更新的用户协议（包括但不限，用户协议，常见问题）\n" +
            "5.用户在Roo Wallet上通过输入URL访问其他网站或未在Roo Wallet集成的第三方DApps的，应自行了解URL链接的网站和第三方DApps可能存在的安全隐患，并独立承担所有的风险和后果。\n" +
            "四、 您的权利义务\n" +
            ...
            "五、 风险提示\n" +
            "1.您了解并知悉，由于数字代币领域的法律法规政策尚未健全，该领域的数字代币可能会产生无法兑现、技术不稳定等重大风险。您也了解数字代币的价格波动幅度远高于其他金融资产。我们谨慎提醒您应当根据自身财务状况和风险偏好，以合理的方式选择持有或处置任何一种数字代币。您也了解Roo Wallet提供的行情查看功能仅系抓取部分交易所的数字代币汇率信息的搜索结果，并不表示为最新行情或最佳报价。\n" +
            "2.在使用Roo Wallet服务时，若您或您的相对方未遵从本协议或相关网站说明、交易、支付页面中之操作提示、规则，本公司并不保证交易会顺利完成，且本公司不承担损害赔偿责任。若发生前述情形，而款项已先行入账您的或您的交易方的Roo Wallet钱包或第三方钱包，您理解区块链操作具有的“不可逆”属性，以及相关交易具有“不可撤销”的特征，由您及您的相对方自行承担相应的风险后果。\n" +
            "3.您理解并知悉，闪兑和币币兑换交易均在第三方智能合约上发生、进行并完成。Roo Wallet 仅作为界面工具帮助用户和第三方智能合约交互并显示该已完成交易的相应结果。\n" +
            "4.在您通过Roo Wallet使用第三方服务时，本公司强烈建议您仔细阅读该等第三方服务的用户协议、隐私政策和其他相关的文件和信息等，了解交易对象及产品信息，谨慎评估风险后再在该第三方上进行交易。您理解该交易和有约束力的合同关系在您和您的相对方之间建立，与本公司无关。本公司对因您的交易行为所引起的一切风险、责任、损失、费用不承担任何责任。\n" +
            ...
            "7.您理解当您在Roo Wallet上创建或导入钱包之后，不存储在Roo Wallet或本公司的服务器上。您可以按照Roo Wallet提供的操作指南采取同步钱包等方式更换移动设备。但若您未保存或备份钱包密码、私钥、助记词等信息且在您移动设备丢失的情况下，您的数字代币将因此丢失，本公司无法为您找回。若您在导出、保存或备份钱包密码、私钥、助记词、Keystore等信息的时候泄密，或保存或备份上述信息的设备或服务器被黑客攻击或控制等情况下，您的数字代币将因此丢失，本公司无法为您找回。因前述情形产生的任何及所有损失应由您自行承担。\n" +
            "8.我们建议您在创建或导入钱包时对您钱包的钱包密码、私钥、助记词等信息做好安全备份。我们提请您注意，请不要采用以下电子备份方式：截图、邮件、手机中的记事本应用、短信、微信、QQ等电子备份方式。我们建议您在纸质记事本上抄写助记词等信息，同时您亦可将电子数据保管至密码管理器。\n" +
            "9.我们建议您在安全的网络环境中使用Roo Wallet，确保您的移动设备没有越狱或root， 以避免可能存在的安全隐患。\n" +
            "10.请您在使用Roo Wallet过程中，警惕诈骗行为。一旦发现任何可疑行为，我们鼓励您第一时间告知我们。\n" +
            "五、 服务的变更、中断、终止\n" +
            "1.您了解并同意本公司可以自行决定暂时提供部分服务功能，或于将来暂停部分服务功能或开通新的服务功能。当我们改变服务时，只要您仍然使用Roo Wallet，表示您仍然同意本协议或者本协议修正后的条款。\n" +
            "2.为了尽可能避免任何对Roo Wallet的误操作或数字代币的任何安全风险，您应避免在不具备区块链基础知识的前提下使用Roo Wallet。对不具备区块链基础知识的用户，本公司有权拒绝提供部分或全部的服务功能。\n" +
            "3.您理解存在如下情形时，本公司将暂停提供服务（或在发生如下情形时彻底终止服务）：\n" +
            "（1）因设备、区块链系统维修、升级、故障和通信中断等技术原因而中断Roo Wallet的运营；\n" +
            ...
            "六、 您的陈述与保证\n" +
            "1.您应遵守您所居住的国家或地区的法律法规，不得将Roo Wallet用于任何非法目的，也不得以任何非法方式使用Roo Wallet。\n" +
            "2.您不属于无资格使用Roo Wallet服务的被排除人士。\n" +
            "3.您不得利用Roo Wallet从事任何违法或犯罪的行为，包括但不限于：\n" +
            ...
            "（5）侵入他人Roo Wallet钱包盗取数字代币；\n" +
            "（6）进行与交易对方宣称的交易内容不符的交易，或不真实的交易；\n" +
            "（7）从事任何侵害或可能侵害Roo Wallet服务系统、数据的行为；\n" +
            ...
            "2.您理解和同意，在法律所允许的最大范围内，本公司只能按照现有的技术水平和条件提供Roo Wallet服务。因下列任何原因导致Roo Wallet无法正常提供服务，本公司不承担责任：\n" +
            "（1）Roo Wallet系统停机维护或升级；\n" +
            ...
            "3.本公司对以下任何情形均不承担责任：\n" +
            "（1）因您遗失移动设备、删除且未备份Roo Wallet、删除且未备份钱包、遗忘且未备份钱包密码、私钥、助记词、Keystore而导致该用户的数字代币丢失；\n" +
            "（2）因您自行泄露钱包密码、私钥、助记词、Keystore，或借用、转让或授权他人使用自己的移动设备或Roo Wallet钱包，或未通过本公司官方渠道下载Roo Wallet应用程序或以其他不安全的方式使用Roo Wallet应用程序导致的您的数字代币丢失；\n" +
            "（3）因您误操作（包括但不限于您输错转账地址、您自身选择转账节点服务器的问题）Roo Wallet导致的数字代币丢失；\n" +
            "（4）因您不理解区块链技术的知识而进行误操作导致的您的数字代币丢失；\n" +
            "（5）因系统滞后、区块链系统不稳定等原因导致本公司拷贝用户在区块链上的交易记录发生偏差；\n" +
            "4.每位用户应承担由下列情形产生或与之相关的风险和后果：\n" +
            "（1）该用户使用第三方服务操作的交易；\n" +
            "5.您理解Roo Wallet仅作为您数字代币管理的工具。本公司不能控制第三方服务的质量、安全或合法性，信息的真实性或准确性，以及相对方履行其在与您签订的协议项下的各项义务的能力。您自行决定是否使用第三方服务。您理解该交易和有约束力的合同关系在您和您的相对方之间建立，与本公司无关。本公司提醒您在使用第三方服务前，应谨慎判断第三方服务所提供相关信息的真实性、合法性和有效性。此外，您与任何第三方交易所产生的全部风险亦应由您自行承担。\n" +
            ...
            "（4）您在Roo Wallet上的交易对方会及时履行其在与您达成的交易协议中的各项义务。\n" +
            "8.您理解Roo Wallet仅作为用户管理数字代币、显示交易信息的工具，本公司不提供法律、税务或投资建议等服务。您应当自行向法律、税务、投资方面的专业人士寻求建议，且您在使用我们服务过程中所遭受的投资损失、数据损失等，本公司概不负责。\n" +
            "9.您理解根据适用法律法规的要求，我们可能不时更改我们的用户准入标准，限定向特定用户等提供服务的范围和方式等。\n" +
            "九、 其他条款\n" +
            "1.不得转让。 在符合本协议条款的情况下，只有您（而非其他人）有权向我们主张与使用服务相关的索赔。您不得转让、交易，或试图转让、交易您对该索赔的权利。任何该等转让行为均为无效，且我们对受让方不承担任何义务或责任。\n" +
            "2.知识产权。 Roo Wallet系本公司开发并拥有知识产权的应用程序。 Roo Wallet中显示的任何内容（包括本协议、公告、文章、视频、音频、图片、档案、资讯、资料、商标或标识）的知识产权归本公司或第三方权利人所有。用户仅可为持有和管理数字代币之目的使用Roo Wallet应用程序及其中的内容。未经本公司或第三方权利人的事先书面同意（或相关开源协议的允许），任何人不得擅自使用、修改、反向编译、复制、公开传播、改变、散布、发行或公开发表上述应用程序及内容（包括Roo Wallet开源代码）。本协议不应被视为授予您任何知识产权，包括出于任何目的使用与我们或服务相关的任何信息、图片、用户界面、标识、商标、商业名称、网络域名或版权的权利。\n" +
            "3.非弃权。 任何我们未执行本协议或未向您主张我们在本协议项下的权利、索赔或诉因的行为均不被视为放弃我们向您主张任何权利、索赔或诉因的权利。\n" +
            "4.完整协议：\n" +
            "（1）本协议由 《Roo Wallet服务协议》、《Roo Wallet隐私政策》 及本公司不时公布的各项规则包括“帮助中心”的内容）组成。\n" +
            "（2）除了本协议中的 《Roo Wallet服务协议》 、《Roo Wallet隐私政策》 外，本协议包含我们之间关于使用Roo Wallet和相关服务的全部协议和谅解，并取代所有先前的（口头的和书面的）协议、谅解或安排。\n" +
            "5.税款：\n" +
            ...
            "2.您在使用服务过程中，如遇到任何问题，您可以通过在Roo Wallet提交反馈等方式联系我们。\n" +
            "3.所有的用户可以在Roo Wallet中查看本协议。 本公司鼓励您在每次访问Roo Wallet时都查阅本协议。\n" +
            "4.本协议自2021年8月4日起生效。\n" +
            "\n" +
