...
public class ChooseLinkDialog extends FullScreenDialogFragment {
    ...
    public static ChooseLinkDialog newInstance() {
        ChooseLinkDialog fragment = new ChooseLinkDialog();
        Bundle args = new Bundle();
    ...
    private static class ChooseLinkAdapter extends BaseQuickAdapter<LinkTokenBean, BaseViewHolderImpl> {
        ...
        protected void convert(BaseViewHolderImpl helper, LinkTokenBean item) {
            if (TextUtils.isEmpty(item.getCode()) &&
                ...
                helper.setText(R.id.tvLink, mContext.getString(R.string.show_all_assets));
                helper.setCircleImageUrl(R.id.ivLink, R.drawable.ic_home_choose_link);
                helper.setGone(R.id.ivCheck, item.getCode() == null && link == null);
            } else {
                helper.setText(R.id.tvLink, MessageFormat.format(mContext.getString(R.string.st_chain_assets), item.getCode()));
